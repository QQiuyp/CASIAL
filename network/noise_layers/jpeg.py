	...
	def forward(self, image_and_cover):
		...
		for i in range(shape[0]):
			...
			with Image.open(buffer) as jpeg_image:
				jpeg = np.array(jpeg_image.convert("RGB"), dtype=np.uint8)
			buffer.close()
		...
		# return (noised_image - original_image).detach() + image
		return noised_image


class KJpegThenJpegTest(nn.Module):
	def __init__(self, Q=50, jpegtest_Q=None, kjpeg_Q=None, subsample=2, path="temp/", straight_through=True, order="kjpeg_then_jpegtest"):
		super(KJpegThenJpegTest, self).__init__()
		kjpeg_source = Q if kjpeg_Q is None else kjpeg_Q
		if isinstance(kjpeg_source, (list, tuple)):
			self.kjpeg_qualities = [int(q) for q in kjpeg_source]
		else:
			self.kjpeg_qualities = [int(kjpeg_source)]
		if not self.kjpeg_qualities:
			raise ValueError("KJpegThenJpegTest requires at least one KJpeg quality.")
		self.Q = tuple(self.kjpeg_qualities) if len(self.kjpeg_qualities) > 1 else self.kjpeg_qualities[0]
		if jpegtest_Q is not None:
			self.jpegtest_Q = int(jpegtest_Q)
		elif isinstance(Q, (list, tuple)):
			self.jpegtest_Q = int(Q[-1])
		else:
			self.jpegtest_Q = int(Q)
		self.straight_through = straight_through
		self.order = str(order).lower()
		if self.order not in {"parallel", "kjpeg_then_jpegtest", "jpegtest_then_kjpeg"}:
			raise ValueError(f"Unsupported KJpegThenJpegTest order: {order}")
		self.kjpegs = nn.ModuleList([KJpeg(Q=q) for q in self.kjpeg_qualities])
		self.jpegtest = JpegTest(Q=self.jpegtest_Q, subsample=subsample, path=path)

	def forward(self, image_and_cover):
		image, cover_image = image_and_cover
		if self.order == "parallel":
			# Parallel bridge: both branches see the same encoded image. The
			# decoder receives all branches, while only differentiable KJpeg
			# contributes a true encoder gradient.
			jpegtest_image = self.jpegtest((image, cover_image))
			if self.straight_through:
				jpegtest_image = jpegtest_image.detach() + image - image.detach()
			branches = [jpegtest_image]
			for kjpeg in self.kjpegs:
				branches.append(kjpeg((image, cover_image)))
			return torch.cat(branches, dim=0)

		if self.order == "jpegtest_then_kjpeg":
			jpegtest_image = self.jpegtest((image, cover_image))
			noised_image = jpegtest_image
			for kjpeg in self.kjpegs:
				noised_image = kjpeg((noised_image, cover_image))
			if self.straight_through:
				return noised_image.detach() + image - image.detach()
			return noised_image

		kjpeg_image = image
		for kjpeg in self.kjpegs:
			kjpeg_image = kjpeg((kjpeg_image, cover_image))
		noised_image = self.jpegtest((kjpeg_image, cover_image))
		if self.straight_through:
			return noised_image.detach() + kjpeg_image - kjpeg_image.detach()
		return noised_image


class JpegSSThenJpegTest(nn.Module):
	def __init__(self, Q=50, jpegtest_Q=None, jpegss_Q=None, subsample=2, path="temp/", straight_through=True):
		super(JpegSSThenJpegTest, self).__init__()
		jpegss_source = Q if jpegss_Q is None else jpegss_Q
		if isinstance(jpegss_source, (list, tuple)):
			self.jpegss_qualities = [int(q) for q in jpegss_source]
		else:
			self.jpegss_qualities = [int(jpegss_source)]
		if not self.jpegss_qualities:
			raise ValueError("JpegSSThenJpegTest requires at least one JpegSS quality.")
		self.Q = tuple(self.jpegss_qualities) if len(self.jpegss_qualities) > 1 else self.jpegss_qualities[0]
		if jpegtest_Q is not None:
			self.jpegtest_Q = int(jpegtest_Q)
		elif isinstance(Q, (list, tuple)):
			self.jpegtest_Q = int(Q[-1])
			...
			self.jpegtest_Q = int(Q)
		self.straight_through = straight_through
		self.jpegsses = nn.ModuleList([JpegSS(Q=q) for q in self.jpegss_qualities])
		self.jpegtest = JpegTest(Q=self.jpegtest_Q, subsample=subsample, path=path)

	def forward(self, image_and_cover):
		image, cover_image = image_and_cover
		jpegss_image = image
		for jpegss in self.jpegsses:
			jpegss_image = jpegss((jpegss_image, cover_image))
		noised_image = self.jpegtest((jpegss_image, cover_image))
		if self.straight_through:
			return noised_image.detach() + jpegss_image - jpegss_image.detach()
		return noised_image
...
class JpegSS(JpegBasic):
	...
	def round_ss(self, x):
		cond = (torch.abs(x) < 0.5).to(dtype=torch.float, device=x.device)
